    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex-1 flex flex-col h-full bg-[#F8FAFC] dark:bg-slate-900 overflow-hidden">
      <div className="flex-1 overflow-y-auto p-6 pt-24 flex flex-col justify-start">
        <div className="text-center mb-10">
          ...
          <div className="my-8 relative"><p className="text-slate-400 font-bold uppercase tracking-widest text-xs mb-2">Total Pago</p><p className="text-7xl font-black text-slate-900 tracking-tighter">{formatCurrency(checkedTotal)}</p><p className="text-sm font-medium text-slate-400 mt-2 bg-slate-100 w-fit mx-auto px-3 py-1 rounded-full">{boughtItems.length} itens no carrinho</p></div>
          {comparisonAvailable && (
        ...
        <div className="bg-white/90 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white mb-24">
          ...
          <input type="text" placeholder="Nome da Lista (ex: Feira Semanal)" className="w-full bg-slate-50 border border-slate-200 p-5 rounded-2xl mb-6 outline-none focus:ring-2 focus:ring-emerald-500 font-bold text-lg" value={listName} onChange={e => onSetListName(e.target.value)} />
          ...
          <button onClick={() => onSaveAndClose(false)} className="w-full py-4 text-slate-400 font-bold hover:text-slate-600 transition-colors">Não salvar modelo, apenas sair</button>
        </div>
      </div>
    </motion.div>
  )
