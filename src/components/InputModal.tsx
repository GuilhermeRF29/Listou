...
export function InputModal({
  ...
  return (
    <>
      ...
      <div className={cn("fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-800/90 backdrop-blur-3xl rounded-t-[2.5rem] z-40 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] border-t border-white dark:border-slate-700", show ? "translate-y-0" : "translate-y-[120%]")}>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-4 overflow-hidden">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white/80 dark:bg-slate-800/80 p-4 rounded-3xl border border-slate-200 dark:border-slate-600 focus-within:ring-2 ring-emerald-500/20 transition-all col-span-2">
                      ...
                      <input type="text" placeholder="Ex: Bebidas, Laticínios" className="w-full bg-transparent outline-none font-bold text-slate-700 dark:text-slate-200 text-lg placeholder:font-normal" value={inputType} onChange={e => onTypeChange(e.target.value)} />
                    </div>
                    <div className="bg-white/80 dark:bg-slate-800/80 p-4 rounded-3xl border border-slate-200 dark:border-slate-600 focus-within:ring-2 ring-emerald-500/20 transition-all">
                      ...
                      <input type="text" placeholder="Ex: Nestle" className="w-full bg-transparent outline-none font-bold text-slate-700 dark:text-slate-200 text-lg placeholder:font-normal" value={inputBrand} onChange={e => onBrandChange(e.target.value)} />
                    </div>
                    <div className="bg-white/80 dark:bg-slate-800/80 p-4 rounded-3xl border border-slate-200 dark:border-slate-600 focus-within:ring-2 ring-emerald-500/20 transition-all">
                      <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide block mb-1">Tamanho</label>
                      <input type="text" placeholder="Ex: 200g" className="w-full bg-transparent outline-none font-bold text-slate-700 dark:text-slate-200 text-lg placeholder:font-normal" value={inputSize} onChange={e => onSizeChange(e.target.value)} />
                    </div>
                    <div className="bg-white/80 dark:bg-slate-800/80 p-4 rounded-3xl border border-slate-200 dark:border-slate-600 focus-within:ring-2 ring-emerald-500/20 transition-all">
                      <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide block mb-1">Mercado</label>
                      <input type="text" placeholder="Ex: Assaí, Extra" className="w-full bg-transparent outline-none font-bold text-slate-700 dark:text-slate-200 text-lg placeholder:font-normal" value={inputStore} onChange={e => onStoreChange(e.target.value)} />
                    </div>
                  </div>
                  <div className="pt-2">
            ...
            <div className="flex gap-4 items-stretch">
              {view !== 'catalog' && (
                <div className="flex flex-col justify-center items-center bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-600 rounded-3xl px-2 py-2 shadow-sm w-20">
                  <button onClick={() => onQtyChange(inputQty + 1)} className="p-3 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/50 rounded-xl transition-colors"><Plus size={20} strokeWidth={3} /></button>
                  <span className="font-black text-2xl text-slate-800 dark:text-slate-100 my-1">{inputQty}</span>
                  <button onClick={() => onQtyChange(Math.max(1, inputQty - 1))} className="p-3 text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl transition-colors"><Minus size={20} strokeWidth={3} /></button>
                </div>
              )}
