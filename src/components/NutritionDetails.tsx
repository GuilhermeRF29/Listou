      ...
      {item && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/30 backdrop-blur-md z-50 flex items-center justify-center p-6" onClick={onClose}>
          <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white/95 backdrop-blur-xl w-full max-w-sm rounded-[2.5rem] p-8 shadow-[0_20px_60px_rgba(0,0,0,0.2)] border border-white/50 relative max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <button onClick={onClose} className="absolute top-6 right-6 bg-slate-100 p-2 rounded-full hover:bg-slate-200"><X size={20} /></button>
            ...
            {item.nutrition && Object.keys(item.nutrition.p100 || {}).length > 0 ? (
              <div className="bg-white/50 rounded-3xl p-6 border border-slate-100">
                <h4 className="font-bold text-slate-700 mb-4 flex items-center gap-2 text-lg"><Leaf size={20} className="text-emerald-500" /> Nutrição{item.nutrition && (() => { const s = calcNutriScore(item.nutrition); return s ? <span className={cn("text-sm font-bold px-2 py-0.5 rounded-md ml-auto", nutriScoreColors[s.grade])}>{s.grade}</span> : null })()}</h4>
                ...
                {fields.map(field => (
                  <div key={field} className="grid grid-cols-3 gap-2 text-sm mb-2 border-b border-slate-100 pb-2 last:border-0 items-center">
                    ...
                    <span className="text-center font-medium text-slate-800">{item.nutrition.p100[field] || '-'}</span>
                    <span className="text-center font-medium text-slate-800">{item.nutrition.portion[field] || '-'}</span>
                  </div>
                ))}
              </div>
            ) : (
