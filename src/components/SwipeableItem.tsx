        ...
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-1 pointer-events-auto shrink-0" onClick={(e) => e.stopPropagation()}>
              ...
              {item.price > 0 && item.size && formatUnitPrice(item.price, item.size) && <span className="text-[10px] font-bold text-slate-400">{formatUnitPrice(item.price, item.size)}</span>}
            </div>
          </div>
        </GlassCard>
      </motion.div>
