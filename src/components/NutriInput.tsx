  ...
  return (
    <div className="flex flex-col gap-1 p-2 bg-slate-50 rounded-xl border border-slate-100">
      ...
      <div className="flex gap-2">
        <input type="text" placeholder="100g" className="w-full bg-white p-1 rounded text-center text-xs border-none outline-none font-bold text-slate-800"
          value={nutrition.p100[field] || ''}
          onChange={e => onChange({ ...nutrition, p100: { ...nutrition.p100, [field]: e.target.value } })} />
        <input type="text" placeholder="Porção" className="w-full bg-white p-1 rounded text-center text-xs border-none outline-none font-bold text-slate-800"
